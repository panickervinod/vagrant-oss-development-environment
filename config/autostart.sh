...
if [ $DISPLAY == ":0" ] ; then
	...
	if [ ! -d "$vibase/vagrant-oss-development-environment" ] ; then
		cd $vibase
	...
	if [ -f "$vibase/vagrant-oss-development-environment/imageupdate.sh" ] ; then
		bash $vibase/vagrant-oss-development-environment/imageupdate.sh
	fi
